...
from sympy import Add, Integral, exp, log, symbols
...
class TimeIntegration01:
    ...
    def _set_ref(self):
        ...
        self.ref = {"time_doit_meijerg": -k * u0 / (k - l) * (exp(t * (l - k)) - 1)}
        ...
        for key, val in self.values.items():
            try:
                ...
                if (ref - val).simplify() != 0:
                    raise ValueError(
                        "Incorrect result, invalid timing: %s != %s" % (ref, val)
                    )
...
class TimeIntegrationPoly:
    params = [5, 10, 20]

    def setup(self, n):
        x = symbols("x")
        expr = Add(*[x**i for i in range(n)])
        self.integral = Integral(expr, x)
